...
由于 Github Pages 并不支持 Pandoc，所以就需要在本地将站点完全生成好后再将它送到某一github上的某一个仓库里。
有人可能觉得麻烦，但也带来几个优点：

+ 首先是因为这样做会使 Github Pages 更新速度大大加快，不会出现今晚提交，明早才被更新好的情况。
+ 其次，可以随心所欲地使用各种插件，各种静态站点生成器……
...
在网站根目录下创建一个名为 *.nojekyll* 的文件就可以关闭服务器端的 Jekyll 编译器。
但是将 *_site* 文件夹上传github的master分支上，网站是不会被发布的。
