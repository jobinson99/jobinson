...
+ 用户或者组织级的，域名是 *用户或组织名.github.io* 。启用方式：建立一个名叫 *xxx.github.io* 的仓库，即可启用，github会自动把xxx.github.io仓库里的文件，转为github page的页面。
+ 项目级的，域名是 *用户或组织名.github.io/项目名* 。启用方式：建立一个名叫 *gh-pages* 的分支，即可启用，github会自动把 gh-pages 仓库里的文件，转为github page的页面。
...
Jekyll提供了一个插件 jekyll-import(需要安装），可以将旧的博客导入到Jekyll。
...
还有python的pelican，emacs的org-page，js的Wintersmith等
如果不想装ruby的时候，可用这些。
